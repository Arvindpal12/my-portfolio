...
const Navbar = () => {
    ...
    return (
        <nav className='flex flex-wrap justify-between md:item-center text-white px-10 pt-6 md:px-20'>
            ...
            <ul className={`${isMenuOpen ? "block" : "hidden"}  mx-24 p-y2 mt-4 font-semibold md:mt-5 bg-black px-2 rounded-xl bg-opacity-30 md:border-none text-center md:bg-transparent md:static md:mx-0 md:flex gap-6`}>
            ...
            {isMenuOpen ? (
                <RiCloseLine size={30} className='md:hidden absolute right-10 top-6 transition-all duration-300 cursor-pointer' onClick={() => setIsMenuOpen(false)} />
            ) : (
                <RiMenu2Line size={30} className='md:hidden absolute right-10 top-6 transition-all duration-300 cursor-pointer' onClick={() => setIsMenuOpen(true)} />
            )}
        </nav>
    )
}
